...
echo -e "DOFLinx will be installed in /recalbox/share/doflinx"
echo -e ""
...
if [[ ! -w "/recalbox/share" ]]; then
    echo -e "${red}[ERROR]${nc} No write permission to /recalbox/share."
...
cd /recalbox/share/doflinx && ./DOFLinx PATH_INI=/recalbox/share/doflinx/config/ &
EOFSCRIPT
...
if [[ -f "$RECALBOX_STARTUP" ]]; then
    ...
    if grep -q "doflinx" "$RECALBOX_STARTUP"; then
        ...
        if grep -q "pixelweb" "$RECALBOX_STARTUP"; then
            ...
            if [ $? -eq 0 ]; then
                ...
                echo -e "    /recalbox/share/doflinx/doflinx.sh &"
            fi
        else
    ...
    echo -e "${yellow}[INFO]${nc} To start manually (after pixelweb is running): /recalbox/share/doflinx/doflinx.sh"
fi
