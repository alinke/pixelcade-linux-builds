...
echo "${magenta}       *** MANUAL START MODE - Pixelcade will NOT auto-start on boot ***${white}"
if [[ "$beta" == "true" ]]; then
...
echo "${yellow}Skipping Pixelcade auto-start setup (Manual Start Mode) - Pixelcade will NOT be added to user-startup.sh${white}"
echo "${yellow}You will start Pixelcade manually over SSH instead - see instructions at the end of this installer${white}"

# If Pixelcade was already added to user-startup.sh from a previous normal install, remove it
# so it doesn't fight with manual startup.
if [[ -f "${INSTALLPATH}linux/user-startup.sh" ]] && grep -q 'runpixelcade' "${INSTALLPATH}linux/user-startup.sh"; then
    echo "${yellow}Removing existing Pixelcade auto-start line from user-startup.sh...${white}"
    sed -i "\#cd ${INSTALLPATH}pixelcade && ./runpixelcade.sh#d" "${INSTALLPATH}linux/user-startup.sh"
fi
...
# Manual Start Mode: we do NOT auto-run Pixelcade here. See instructions at the end
# of this installer for how to start it manually over SSH.
...
echo ""
echo "${magenta}=========================================================${white}"
echo "${magenta}   INSTALLATION COMPLETE - MANUAL START MODE${white}"
echo "${magenta}=========================================================${white}"
echo ""
echo "${yellow}Pixelcade has been installed but will NOT start automatically on boot.${white}"
echo "${yellow}To start it manually, SSH into your MiSTer and run it yourself:${white}"
echo ""
echo "  1. From another computer, SSH into your MiSTer:"
echo "       ${green}ssh root@<your-mister-ip>${white}"
echo "     (default password is 1, unless you've changed it)"
echo ""
echo "  2. Go to the Pixelcade install folder:"
echo "       ${green}cd ${INSTALLPATH}pixelcade${white}"
echo ""
echo "  3. Run Pixelcade in the foreground so you can see any errors/crash output:"
echo "       ${green}./runpixelcade.sh${white}"
echo "     (or run ${green}./pixelweb${white} directly for even more verbose output)"
echo ""
echo "  4. Leave that SSH session open while you test - Pixelcade will keep running"
echo "     until you close the session or press Ctrl+C."
echo ""
echo "${yellow}Since it's not in user-startup.sh, a reboot will NOT relaunch Pixelcade -${white}"
echo "${yellow}you'll need to SSH back in and run ./runpixelcade.sh again each time.${white}"
echo ""
...
while true; do
    ...
    case $yn in
        ...
        [Nn]* ) echo "${yellow}Please reboot when you get a chance, then SSH in and run ./runpixelcade.sh manually as shown above${white}" && exit;;
        * ) echo "Please answer yes or no.";;
    esac
done